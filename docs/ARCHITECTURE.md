...
- **Frontend (P039):** plain HTML, CSS and JavaScript modules in `public/`, served by the same Express app — no build step, so it works identically from `src/` (tsx) and `dist/`. A small hash router (`public/js/app.js`) keeps the view in the URL (`#/runs?status=fail&page=2`), so reload and the back button work. All data is inserted with `textContent` (never `innerHTML`), so page names, notes and URLs from runs can't inject HTML
  ...
  - **Accept and restore (P043):** the run detail page has an "Accept these changes" panel (whole run) and an "Accept this page" button on each changed page, calling `POST /api/runs/:id/accept`. The **Baselines** page (`#/baselines/:tag`) lists every version with how it was made and restores archived ones (`POST /api/baselines/:tag/restore`); `#/baselines/:tag/v3` shows one version's screenshots. Actions confirm in the page (no browser dialogs), show progress, and report the outcome in place; a refusal says nothing changed and offers "Try again". When a capture has failed screenshots the API answers 409 with `code: "failed_screenshots"`, and the dashboard offers to accept just the screenshots that worked (`force: true`)
  - **API client (`public/js/api.js`, P043):** every call turns HTTP, network and timeout failures into an `ApiError` with a readable message (plus validation `issues` and a machine-readable `code`). Reads time out after 20s and are retried once after a network error or 502/503/504; actions are never retried and allow 2 minutes. The router gives each page an `AbortSignal` and cancels it on navigation, so a slow response can't draw over a newer page. Error pages list validation issues and have a "Try again" button
...
- **Presenting (P049):** [DEMO.md](DEMO.md) is a presenter's walkthrough (checklist, 5- and 10-minute talk tracks, recovery plan, common questions).
  - `npm run demo` takes `--step` (pause before each step in a terminal), `--dashboard [--port]` (serve the results afterwards) and `--scripted-judge`.
  - `--scripted-judge` swaps the AI for `examples/demo-site/scriptedJudge.ts`, which has pre-written verdicts for the demo pages and makes no API calls. It reports itself as `scripted-demo-judge` in the console, report and dashboard, so it can't pass for the real model.
  - `npm run demo:site` serves the shop on its own, and `?version=1|2` shows either version for one page view.
  - `tests/demo.test.ts` runs the demo script as a real process (output in a temp folder via `PIXELGUARD_DEMO_OUT`).
