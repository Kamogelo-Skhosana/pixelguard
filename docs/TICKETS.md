...
- [x] **P014** — Add unit tests for the diff engine using sample image pairs

### Epic: CLI & Raw Report Output

- [x] **P015** — Build CLI entry point: `pixelguard capture` and `pixelguard diff` commands
- [x] **P016** — Implement console output formatter for diff summaries
- [x] **P017** — Implement JSON export of diff results (`--output diffs.json`)
...
- [x] **P020** — Add a CLI option to supply a short description of what changed in this build (helps the LLM's judgment)
...
- [x] **P023** — Implement the judgment call per diff and parse the structured response
- [x] **P024** — Extend `DiffResult` with verdict, confidence score, and explanation fields
- [x] **P025** — Add unit tests using mocked LLM responses (no live API calls in CI)

### Epic: Verdict Aggregation

- [x] **P026** — Implement per-page verdict rollup (does this page pass or fail overall?)
- [x] **P027** — Implement run-level summary (X real bugs, Y acceptable changes, Z uncertain)
- [x] **P028** — Add tests covering aggregation edge cases (mixed verdicts on one page, all-uncertain runs)
...
- [x] **P033** — Implement save-run-to-database logic after each run
...
- [x] **P036** — Implement `GET /runs` — list run history
- [x] **P037** — Implement `GET /runs/:id` — page diffs detail for one run
- [x] **P038** — Implement `GET /runs/trend` — regression rate over time
...
- [x] **P049** — Prepare a demo script / sample site walkthrough for presenting the project
- [x] **P050** — Final end-to-end test pass across all three phases; tag `v1.0` release
